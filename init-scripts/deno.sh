...
DATABASE_DIR="${DATABASE_DIR_DENO:-/data/db/sqlite}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set webroot
WWW_ROOT_DIR="/usr/share/webapps/deno"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Default predefined variables
DATA_DIR="/data/deno"   # set data directory
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TMP_DIR="/tmp/deno"
RUN_DIR="/run/deno"       # set scripts pid dir
...
SERVICE_USER="deno"  # execute command as another user
SERVICE_GROUP="deno" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
[ -f "$CONF_DIR/env/deno.sh" ] && . "$CONF_DIR/env/deno.sh"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__update_conf_files() {
  ...
  local server_files="$(find "$WWW_ROOT_DIR" "$WWW_ROOT_DIR/src" -maxdepth 1 -type f -iname 'app.ts' -iname 'server.ts' -iname'index.tsx' | head -n1)"
  ...
  # custom commands
  [ -d "$WWW_ROOT_DIR" ] && cd "$WWW_ROOT_DIR" || exit 1
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# root/admin user info [password/random]
#ENV_ROOT_USER_NAME="${ENV_ROOT_USER_NAME:-$DENO_ROOT_USER_NAME}"   # root user name
#ENV_ROOT_USER_PASS="${ENV_ROOT_USER_NAME:-$DENO_ROOT_PASS_WORD}"   # root user password
#root_user_name="${ENV_ROOT_USER_NAME:-$root_user_name}"                              #
#root_user_pass="${ENV_ROOT_USER_PASS:-$root_user_pass}"                              #
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#Normal user info [password/random]
#ENV_USER_NAME="${ENV_USER_NAME:-$DENO_USER_NAME}"                  #
#ENV_USER_PASS="${ENV_USER_PASS:-$DENO_USER_PASS_WORD}"             #
