...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
grep -qsi "$EXPECTED_OS" /etc/*.release* || { echo "" && exit 1; }
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
curl -q -LSsf -o "$TMP_FILE" "$LATEST" 2>/dev/null && URL="$LATEST" || curl -q -LSsf -o "$TMP_FILE" "$URL" 2>/dev/null || exitStatus=1
if [ -z "$exitStatus" ] && [ -f "$TMP_FILE" ]; then
...
git clone -q "https://github.com/casjay-templates/denojs" "/usr/share/webapps/deno"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# End application
